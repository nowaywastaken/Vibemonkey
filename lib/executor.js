...
async function findAndObserveElement(tabId, selectors, timeout = 5000) {
    ...
    return await chrome.scripting.executeScript({
        ...
        func: async (candidateSelectors, waitTimeout) => {
            const cleanSelectors = Array.isArray(candidateSelectors) 
                ? candidateSelectors.map(s => ({ type: String(s.type), value: String(s.value) }))
                : [];
            const cleanTimeout = Number(waitTimeout) || 5000;
        ...
        args: [JSON.parse(JSON.stringify(selectors)), timeout]
...
async function executeFill(tabId, step) {
    ...
    const safeSel = String(locator.selectorUsed || "");
    const safeType = String(locator.type || "css");
    const safeVal = String(step.value ?? "");
...
async function executeClick(tabId, step, result) {
    ...
    const execResult = await chrome.scripting.executeScript({
        ...
        args: [
            locator.selectorUsed ? String(locator.selectorUsed) : "", 
            locator.type ? String(locator.type) : "css"
        ]
    });
